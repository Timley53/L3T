...
contract L3Token is ERC20, Ownable, AccessControl, Pausable {
     ...
     * this mappings can be used to avoid a user minting to a single address twice, if balance was used, a user could mint to themselves twice if they have spent their initial minted token.
     */
    ...
    modifier notAddressZero(address addressToCheck) {
        require(addressToCheck != address(0), "invalid address submitted");
        _;
    }
     ...
     */
    function grantMinterRole(address minter) public onlyRole(getRoleAdmin(MINTER_ROLE)) onlyOwner returns (bool) {
     ...
     */
    function userApproveStaker() public notAddressZero(msg.sender) onlyTokenHolder(msg.sender) whenNotPaused {
        // max allowance given to staking contract
        approve(L3TStaker, type(uint256).max);
    }
